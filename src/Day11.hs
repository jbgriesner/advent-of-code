...
proceedMonkey :: MonadLogger m => Int -> Int -> (Map Int [Int], Map Int Int) -> MonkeyRule -> m (Map Int [Int], Map Int Int)
proceedMonkey k mmm itemsMap mr =
    let
        itemsMonkey = (fst itemsMap) ! (id mr)
        numberInspections = length itemsMonkey
        previousInspections = (snd itemsMap) ! (id mr)
        operA = map (operation mr) itemsMonkey
        newWorryLevels = if k == 1 then map (`div` 3) operA else map (`mod` mmm) operA
...
app :: MonadLogger m => Int -> Int -> Int -> [MonkeyRule] -> (Map Int [Int], Map Int Int) -> m (Map Int [Int], Map Int Int)
app kcode mmm k mrs itemsMap = iterateM' (k-1) app' itemsMap
    where
        app' :: MonadLogger m => (Map Int [Int], Map Int Int) -> m (Map Int [Int], Map Int Int)
        app' itemsMap = do
            -- logDebugN (T.pack ".")
            if kcode == 1
               then foldM (proceedMonkey 1 mmm) itemsMap mrs
               else foldM (proceedMonkey 2 mmm) itemsMap mrs
...
code1 s = do
    ...
    rounds <- app 1 mmm 20 monkeyRules (initMap, Map.map (const 0) initMap)
...
code2 s = do
    ...
    rounds <- app 2 mmm 10000 monkeyRules (initMap, Map.map (const 0) initMap)
